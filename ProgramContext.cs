...
namespace MwmBuilder
{
  public class ProgramContext
  {
    ...
    private static string m_materialsPath = @"C:\Users\patgr\Desktop\MyMWMBuilder2\MwmBuilder\bin\Content\Materials";
    ...
    public int Work(object data, IMyBuildLogger[] loggers = null)
    {
      ...
      ProgramContext.m_logger.AddLogger((IMyBuildLogger) ProgramContext.m_fileLogger);
      this.m_watch = new Stopwatch();
      this.m_watch.Start();
      this.m_args.RegisterArg("s", "SOURCE", (string) null, "Path to source FBX file(s), directory or file. Directories are read recursively. Defaults to current directory.");
      this.m_args.RegisterArg("o", "OUTPUT", "C:\\Program Files (x86)\\Steam\\SteamApps\\common\\SpaceEngineers\\Content\\", "Path to output");
      this.m_args.RegisterArg("m", "MASK", "*.fbx", "File mask of files to process, defaults to *.FBX");
      this.m_args.RegisterArg("l", "LOGFILE", (string) null, "Path to logfile");
      this.m_args.RegisterArg("la", "LOGFILE", (string) null, "Path to logfile");
      this.m_args.RegisterArg("t", "THREADS", "1", "Run model build on several threads");
      ...
      this.m_args.RegisterArg("checkOpenBoundaries", (string) null, (string) null, "Warn if model contains open boundaries");
      ...
      this.m_args.RegisterArg("d", "LODS", (string) null, "Float values separated by space, defining default values for LOD 1-n");
      this.m_args.RegisterArg("do", "LODSOverride", (string) null, "Float values separated by space, defining overriden values for LOD 1-n");
      ...
      try
      {
        ...
        {
          ProgramContext.m_logger.LogMessage(MessageType.Error, "Cannot find output path: " + this.m_args.GetValue("o"), "");
          return 1;
        }
        ...
        bool append = this.m_args.GetValue("la") != null;
        if (this.m_args.GetValue("l") != null || this.m_args.GetValue("la") != null)
        {
          string path = append ? this.m_args.GetValue("la") : this.m_args.GetValue("l");
        ...
        this.setMaterialsPathForSource(this.m_args.GetValue("s"));
        ...
        if (this.m_args.GetValue("t") != null)
        {
          ...
          if (int32 > 1)
          {
            int num = new MyWorkDispatcher((IEnumerable<string>) strArray, this.m_args.GetValue("o"), (string) null, this.m_args.GetValue("f") != null, this.m_args.GetValue("g") != null, this.m_args.GetValue("e") != null, this.m_args.GetValue("checkOpenBoundaries") != null, this.m_args.GetValue("d")).Run(int32, this.m_args.GetValue("u") != null) ? 0 : 1;
    ...
    private void ProcessFileSafe(string file)
    {
      try
      {
        ...
        if (this.m_args.GetValue("do") != null)
          overrideLods = true;
        if (this.ProcessFile(file, ProgramContext.OutputDir = this.m_args.GetValue("o"), ProgramContext.m_vars, this.m_args.GetValue("e") != null, this.m_args.GetValue("f") != null, this.m_args.GetValue("checkOpenBoundaries") != null, overrideLods ? this.m_args.GetValue("do") : this.m_args.GetValue("d"), overrideLods))
        {
          lock (ProgramContext.m_logger)
            ProgramContext.m_logger.LogMessage(MessageType.Processed, string.Format("{1}: Finished in {0}s", (object) this.m_watch.Elapsed.TotalSeconds.ToString("f1"), this.m_args.GetValue("i") == null ? (object) "" : (object) this.m_args.GetValue("i")), file);
        }
        ++this.m_processed;
      }
      catch (Exception ex)
    ...
    public void WorkThread(object data) => this.Work(data);
    ...
    private string[] LoadFiles(string src, string mask)
    {
      ...
      foreach (string path in str.Split(chArray))
      {
        if ((File.GetAttributes(path) & FileAttributes.Directory) == (FileAttributes) 0)
        {
          if (File.Exists(path))
            ...
            ProgramContext.m_logger.LogMessage(MessageType.Error, string.Format("Target file '{0}' does not exists", (object) path), "");
        }
        else
        {
      ...
      return ((IEnumerable<string>) stringList.ToArray()).Select<string, FileInfo>((Func<string, FileInfo>) (s => new FileInfo(s))).OrderByDescending<FileInfo, long>((Func<FileInfo, long>) (s => s.Length)).Select<FileInfo, string>((Func<FileInfo, string>) (s => s.FullName)).ToArray<string>();
    }
      ...
      bool overrideLods)
    {
      if (item.Configuration == null)
        ProgramContext.m_logger.LogMessage(MessageType.Info, string.Format("Model skipped! No configuration for '{0}'", (object) item.Path), "");
      else
        new MyModelBuilder().Build(item.Path, "tmp", outputDir, item.Configuration, havokCollisionShapes, checkOpenBoundaries, lodDistances, overrideLods, new Func<string, MyMaterialConfiguration>(ProgramContext.GetMaterialByRef), (IMyBuildLogger) ProgramContext.m_logger);
    }
