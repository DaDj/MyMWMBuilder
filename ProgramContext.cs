    ...
    {
      {
        ...
        private void ProcessFileSafe(string file)
        {
            try
            {
                ...
                if (m_args.GetValue("do") != null)
                    overrideLods = true;
                if (ProcessFile(file, ProgramContext.OutputDir = m_args.GetValue("o"), ProgramContext.m_vars, m_args.GetValue("e") != null, m_args.GetValue("f") != null, m_args.GetValue("checkOpenBoundaries") != null, overrideLods ? m_args.GetValue("do") : m_args.GetValue("d"), overrideLods))
                {
          ...
          bool overrideLods)
        {
            ...
            ProcessItem(itemInfo, outputDir, havokCollisionShapes, checkOpenBoundaries, lodDistances1, overrideLods);
            if (exportXml)
            {
          ...
          bool overrideLods)
        {
            if (item.Configuration == null)
                ...
                new MyModelBuilder().Build(item.Path, "tmp", outputDir, item.Configuration, havokCollisionShapes, checkOpenBoundaries, lodDistances, overrideLods, new Func<string, MyMaterialConfiguration>(ProgramContext.GetMaterialByRef), m_logger);
        }
